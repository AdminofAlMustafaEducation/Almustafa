...
import { chatAgents } from "@/data/chat-agents";
import { notifications } from "@/data/notifications";

export function WhatsAppChat() {
  const [isOpen, setIsOpen] = useState(false);
  const activeAgents = chatAgents
  ...
  function handleAgentClick(agent: (typeof activeAgents)[0]) {
    ...
    window.open(`https://wa.me/${agent.whatsappNumber.replace(/[^0-9]/g, "")}?text=${message}`, "_blank");
    setIsOpen(false);
  }
          ...
          >
            ...
            <div className="bg-green-600 px-4 py-3">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="rounded-full p-1 text-white/80 hover:bg-green-700 hover:text-white"
                >
                  <X className="h-4 w-4" />
                </button>
              </div>
            ...
            <div className="p-3">
              <p className="mb-2 text-[11px] text-gray-500">
                Select an agent to chat on WhatsApp:
              </p>
