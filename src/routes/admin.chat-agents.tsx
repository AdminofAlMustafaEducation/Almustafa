...
function AdminChatAgents() {
  ...
  function handleSave() {
    ...
    if (editingAgent) {
      updateAgent.mutate(
        { id: editingAgent.id, ...formData, photo_url: formData.photo_url || undefined },
        callbacks,
      );
    } else {
      createAgent.mutate(
        { ...formData, photo_url: formData.photo_url || undefined, sort_order: agents.length + 1 },
        callbacks,
      );
    }
  }
        ...
        <CardContent className="flex items-start gap-3 p-4">
          ...
          <div className="text-sm text-green-800">
            ...
            <p className="mt-1 text-green-700">
              Active agents appear in the WhatsApp chat widget on all pages. If multiple agents are
              active, visitors can choose who to chat with.
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSave} disabled={!formData.name || !formData.whatsapp_number}>
