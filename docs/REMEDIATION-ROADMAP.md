...
Before the external rollout, the owner must provision a disposable Supabase environment, run migrations `022`–`029` in order, execute the expanded pgTAP suite, configure server-only Vercel variables, and approve production deployment separately. The public tracking contract is now intentionally admission-code-only and returns only status metadata; this is less private than token-bound tracking, so admission-code format entropy and edge rate limiting must be monitored. Migration `029_reassert_profile_admin_rls.sql` restores profile-based admin RLS and explicit authenticated grants for the admin student page, removing stale legacy policies that could otherwise block or expose the wrong paths. The admin student insert now sends only canonical Supabase columns and surfaces the actual database error. The Vercel project currently reports Node `24.x`, which satisfies the repository’s declared `>=22 <25` range. The live deployment at the time of audit was still the prior pushed commit and requires the new commit to deploy. The local database test remains blocked until Docker/Postgres is available; the attempted `supabase test db` connection to `127.0.0.1:54322` failed because no local database was running. The approval endpoint still requires server-only `SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY` (or `SUPABASE_SECRET_KEY`) in Vercel; the browser session could not access the Vercel login to configure them.
